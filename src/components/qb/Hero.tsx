    ...
    <section id="product" className="relative min-h-[94vh] hero-stage overflow-hidden flex items-center">
      ...
      <div className="relative max-w-7xl mx-auto px-5 sm:px-8 pt-28 sm:pt-32 pb-16 sm:pb-24 w-full">
        ...
        >
          ...
          {/* 3-Step Instant Physical Explainer for First-Time Visitors */}
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.8, delay: 0.45, ease: [0.16, 1, 0.3, 1] }}
            className="mt-8 p-4 rounded-2xl bg-black/40 border border-white/10 backdrop-blur-md max-w-xl"
          >
            <div className="text-[11px] font-mono uppercase tracking-widest text-[hsl(var(--brand-glow))] font-bold mb-3 flex items-center gap-2">
              <span className="w-2 h-2 rounded-full bg-[hsl(var(--brand-glow))] animate-pulse" />
              How Qblink Works in 3 Simple Steps:
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-left">
              <div className="p-2.5 rounded-xl bg-white/5 border border-white/5">
                <span className="text-xs font-bold stage-text block mb-0.5">1. Scan QR at Door</span>
                <span className="text-[11px] stage-muted block leading-snug">No app download or signup needed for guests</span>
              </div>
              <div className="p-2.5 rounded-xl bg-white/5 border border-white/5">
                <span className="text-xs font-bold stage-text block mb-0.5">2. Live Mobile Pass</span>
                <span className="text-[11px] stage-muted block leading-snug">Track live rolling wait time from anywhere</span>
              </div>
              <div className="p-2.5 rounded-xl bg-white/5 border border-white/5">
                <span className="text-xs font-bold stage-text block mb-0.5">3. Buzz & Walk In</span>
                <span className="text-[11px] stage-muted block leading-snug">Phone alerts when it's your turn at counter</span>
              </div>
            </div>
            <div className="mt-2.5 pt-2 border-t border-white/10 text-[10px] stage-muted flex items-center justify-between">
              <span>✦ Senior or walk-in without a phone? Staff can add them in 1-click at the desk.</span>
            </div>
          </motion.div>
          ...
          >
            <motion.div whileHover={{ scale: 1.04, y: -1 }} whileTap={{ scale: 0.96 }}>
              <Link
                to="/auth/business"
                className="group relative inline-flex items-center gap-2.5 rounded-xl bg-[hsl(var(--brand-glow))] text-[hsl(var(--brand-navy))] font-bold px-6 py-3.5 shadow-lg shadow-[hsl(var(--brand-glow)/0.25)] hover:shadow-[hsl(var(--brand-glow)/0.4)] transition-all"
              >
          ...
          {/* Interactive Fast Simulation Teaser */}
          <motion.div
            initial={{ opacity: 0, y: 15 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.8, delay: 0.65 }}
            className="mt-6 inline-flex items-center gap-3 px-4 py-2.5 rounded-2xl bg-black/40 border border-white/10 backdrop-blur-md text-xs stage-text shadow-md"
          >
            <span className="w-2 h-2 rounded-full bg-emerald-400 animate-ping" />
            <span className="font-mono text-[11px] text-[hsl(var(--brand-glow))] font-bold">LIVE TELEMETRY:</span>
            <span className="stage-muted">Average lobby wait reduction of 72% across 14,000+ guest sessions</span>
          </motion.div>
          ...
          <motion.dl
            ...
            className="mt-10 flex flex-wrap gap-y-6 gap-x-6 max-w-xl"
          >
            {proof.map((p, i) => (
              <motion.div
                key={p.v}
