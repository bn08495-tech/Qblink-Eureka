...
import { Clock, Users, CheckCircle, Loader2, Activity, RefreshCw, UserCircle2, AlertCircle, ArrowLeft, Zap } from "lucide-react";
          ...
          <div className="bg-card rounded-2xl p-6 card-shadow">
            ...
            {openState.status === "closed" || openState.status === "holiday" ? (
              ...
              <>
                ...
                {prefilled && visitorName.trim() && phone.trim() && (
                  <div className="mb-5 rounded-2xl bg-gradient-to-br from-primary/15 via-primary/5 to-transparent border border-primary/30 p-4">
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-2.5">
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="text-sm font-bold text-foreground truncate">{visitorName}</p>
                        </div>
                      </div>
                      <span className="text-[10px] uppercase font-bold text-primary px-2 py-0.5 rounded-full bg-primary/10">1-Tap Fast Join</span>
                    </div>
                    <button
                      type="button"
                      onClick={handleJoin}
                      disabled={joining}
                      className="w-full gradient-bg text-primary-foreground py-3 rounded-xl text-sm font-bold hover:opacity-90 active:scale-[0.99] transition-all shadow-md flex items-center justify-center gap-2"
                    >
                      <Zap className="w-4 h-4" />
                      <span>{joining ? "Joining..." : `Quick Join as ${visitorName.split(" ")[0]}`}</span>
                    </button>
                  </div>
                )}
